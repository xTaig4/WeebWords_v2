...
const MainLayout: React.FC = () => {
  ...
  return (
    <div
      className="fixed top-0 bottom-0 flex h-screen w-15 flex-col items-center gap-5"
      style={{ background: "var(--container-bg)", contain: "layout" }}
    >
      {/* SideBar */}
        ...
        <nav className="mt-5 flex flex-col gap-4 text-black">
          {links.map((link, index) => (
            <Link
              key={index}
              href={link.href}
              className="my-1 h-10 w-10 cursor-pointer rounded-lg border border-gray-300 p-1 text-center font-bold"
      ...
      <div className="flex flex-1 flex-col">
        {/* Top search bar */}
        <div
          className="fixed top-0 right-0 left-1/32 w-screen p-5"
